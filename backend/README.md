...
| `MONTHLY_MESSAGE_LIMIT` | 月あたりメッセージ上限。未設定時は 3000。 |
...
7. 既存 DB の場合は、`migrations/001_add_selected_providers.sql` と `migrations/002_add_expanded_from_id.sql` を実行する。
...
- `POST /chat/rooms/:roomId/messages/expand` — 認証必須。body: `{ "message_id": "..." }`。Gemini の「さらに詳しく」用。標準モデルで再回答し、元メッセージの下に追加する。
